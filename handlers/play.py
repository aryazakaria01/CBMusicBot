...
async def playlist(client, message):
    ...
    msg = "**sᴏɴɢ ᴄᴜʀʀᴇɴᴛʟʏ ᴘʟᴀʏɪɴɢ** ᴏɴ {}".format(message.chat.title)
    msg += "\n• "+ now_playing
    msg += "\n• ᴜᴘᴏɴ ʀᴇǫᴜᴇsᴛ "+by
    temp.pop(0)
    if temp:
...
def updated_stats(chat, queue, vol=100):
    if chat.id in callsmusic.pytgcalls.active_calls:
        ...
        if len(que) > 0:
            ...
            stats += "ᴘʟᴀʏɪɴɢ ᴀ sᴏɴɢ: **{}**\n".format(queue[0][0])
            stats += "ᴜᴘᴏɴ ʀᴇǫᴜᴇsᴛ: {}".format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def hfmm(_, message):
    ...
    if len(message.command) != 2:
        await message.reply_text(
            "**ɪ'ᴍ ᴏɴʟʏ ᴋɴᴏᴡ** `/musicplayer on` **ᴀɴᴅ** `/musicplayer off`"
        )
        return
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        if not queue:
            await cb.message.edit("**✘ ɴᴏᴛʜɪɴɢ ɪs ᴘʟᴀʏɪɴɢ ❗**")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "**ɴᴏᴡ ᴘʟᴀʏɪɴɢ** ɪɴ {}".format(cb.message.chat.title)
        msg += "\n➤ " + now_playing
        msg += "\n➤ ʀᴇǫᴜᴇsᴛ ʙʏ " + by
        temp.pop(0)
        if temp:
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style="md")
        msg = "**sᴏɴɢ ᴄᴜʀʀᴇɴᴛʟʏ ᴘʟᴀʏɪɴɢ** ᴏɴ {}".format(cb.message.chat.title)
        msg += "\n➤ "+ now_playing
        msg += "\n➤ ᴜᴘᴏɴ ʀᴇǫᴜᴇsᴛ "+by
        temp.pop(0)
        if temp:
...
async def play(_, message: Message):
    ...
    try:
        ...
        await lel.edit(
            f"<i>{user.first_name} ᴡᴀs ʙᴀɴɴᴇᴅ ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ, ᴀsᴋ ᴀᴅᴍɪɴ ᴛᴏ ᴜɴʙᴀɴ @{ASSISTANT_NAME} manually.</i>"
        )
        return
    text_links=None
    ...
    if audio:
        ...
        try:
          ...
          await lel.edit("**ᴘʟᴇᴀsᴇ ɢɪᴠᴇ ᴀ sᴏɴɢ ɴᴀᴍᴇ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴘʟᴀʏ !**")
        # veez project
        try:
            ...
            while j < 6:
                ...
                toxxt += f" └ ☣ ᴘᴏᴡᴇʀᴇᴅ ʙʏ {BOT_NAME} ᴀ.ɪ\n\n"
                j += 1            
            keyboard = InlineKeyboardMarkup(
                [
                    [
            ...
            await lel.edit("__ɴᴏ ᴍᴏʀᴇ ʀᴇsᴜʟᴛs ᴛᴏ ᴄʜᴏᴏsᴇ, sᴛᴀʀᴛɪɴɢ ᴛᴏ ᴘʟᴀʏɪɴɢ...__")
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            photo="final.png",
            caption=f"✯ **ᴛɪᴛʟᴇ:** [{title[:30]}]({url})\n⌛ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n☣ **sᴛᴀᴛᴜs:** `ǫᴜᴇᴜᴇᴅ ᴘᴏsɪᴛɪᴏɴ {position}`\n" \
        ...
        try:
            ...
            message.reply("**✘ ᴠᴏɪᴄᴇ ᴄʜᴀᴛ ɢʀᴏᴜᴘ ɴᴏᴛ ᴀᴄᴛɪᴠᴇ, ᴄᴀɴ'ᴛ ᴘʟᴀʏ ᴀ sᴏɴɢ.**")
            return
        await message.reply_photo(
            photo="final.png",
            caption=f"✯ **ᴛɪᴛʟᴇ:** [{title[:30]}]({url})\n⌛ **ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n☣ **sᴛᴀᴛᴜs:** `ᴘʟᴀʏɪɴɢ`\n" \
...
async def lol_cb(b, cb):
    ...
    try:
        ...
        await cb.message.edit("✘ sᴏɴɢ ɴᴏᴛ ғᴏᴜɴᴅ")
        return
    useer_id = int(useer_id)
    if cb.from_user.id != useer_id:
        await cb.answer("✘ ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴘᴇᴏᴘʟᴇ ᴡʜᴏ ʀᴇǫᴜᴇsᴛᴇᴅ ᴛʜɪs sᴏɴɢ!", show_alert=True)
        return
